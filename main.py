...
# Checking to see if there are 3 in a row within the rows
...
# Checking to see if there are 3 in a row within the columns
...
# Checking to see if there are 3 in a row when it comes to diag 1
...
# Checking to see if there are 3 in a row when it comes to diag 2
...
# Function to detect computer's move
def move(board):
    while True:
        ai_x = randint(0, 2)
        ai_y = randint(0, 2)
...
# Reruns reiterations of the code in which the player and the computer make moves
while True:
